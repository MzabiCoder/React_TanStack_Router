...
const IdeaCard = ({ idea, button = true }: { idea: Idea, button?: boolean }) => {
    ...
    return <div key={idea._id} className='p-4 border border-gray-300 rounded shadow bg-white flex flex-col justify-between'>
        <h2 className='text-lg font-semibold'>{idea.title}</h2>
        <p className='text-gray-700 mt-2'>{idea.summary}</p>
        <Link to={`/ideas/$ideaId`} params={{ ideaId: idea._id }} className={Style}>
